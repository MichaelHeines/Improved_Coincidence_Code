...
Read_out Reading_dat(TString dat_name, TString data_dir, TString write_dir, TString output_file, TString coincidences_filename, TString singles_filename, TString datatype_chain, TString datatype_tree, Double_t month_0, Double_t day_0, Double_t hour_0, Double_t half_life, Double_t E0, Double_t E1, Double_t ratio_to_activity, Double_t ns_window, Double_t coinc_to_total, Int_t nr_dirs, Int_t cutted_events, Int_t window_events, Int_t minimal_coinc, Int_t hist_bins, std::vector<TString> *ls_dirs, std::vector<TString> *indices, std::vector<Double_t> *months, std::vector<Double_t> *days, std::vector<Double_t> *hours, std::vector<Double_t> calibrations, std::vector<Int_t> ranges, std::vector<Int_t> hwidth, bool process_raw) {
	...
	try {
		...
		std::getline(infile, line);
		try{
			stringstream(line.substr(0, line.find(delimiter))) >> month_0;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> day_0;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> hour_0;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> minute_0;
		...
		try{
			for (Int_t i = 0; i < nr_dirs; i++){
				stringstream(line.substr(0, line.find(delimiter))) >> temp;
				if(i == 0){
					(*months)[i] = temp;
				}
				else{
		...
		std::getline(infile, line);
		try{
			stringstream(line.substr(0, line.find(delimiter))) >> half_life;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> E0;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> E1;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> ratio_to_activity;
			line.erase(0, line.find(delimiter) + delimiter.length());
		}
		catch(...){
		...
		try{
			for (Int_t i = 0; i < 3; i++){
				stringstream(line.substr(0, line.find(delimiter))) >> calibrations[i];
				line.erase(0, line.find(delimiter) + delimiter.length());
			}
		}
		catch(...){
		...
		try{
			for (Int_t i = 0; i < 6; i++){
				stringstream(line.substr(0, line.find(delimiter))) >> ranges[i];
				line.erase(0, line.find(delimiter) + delimiter.length());
			}
		}
		catch(...){
		...
		try{
			for (Int_t i = 0; i < 2; i++){
				stringstream(line.substr(0, line.find(delimiter))) >> hwidth[i];
				line.erase(0, line.find(delimiter) + delimiter.length());
			}
		}
		catch(...){
		...
		std::getline(infile, line);
		try{
			stringstream(line.substr(0, line.find(delimiter))) >> cutted_events;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> window_events;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> minimal_coinc;
			line.erase(0, line.find(delimiter) + delimiter.length());
		}
		catch(...){
		...
		line.erase(line.size() - 1, line.size());
		try{
			stringstream(line.substr(0, line.find(delimiter))) >> datatype_chain;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> datatype_tree;
			line.erase(0, line.find(delimiter) + delimiter.length());
		}
		catch(...){
		...
		std::getline(infile, line);
		try{
			stringstream(line.substr(0, line.find(delimiter))) >> hist_bins;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> ns_window;
			line.erase(0, line.find(delimiter) + delimiter.length());
			stringstream(line.substr(0, line.find(delimiter))) >> coinc_to_total;
			line.erase(0, line.find(delimiter) + delimiter.length());
		}
		catch(...){
			std::cerr << "ERROR: Number of datatypes is not appropriate.";
