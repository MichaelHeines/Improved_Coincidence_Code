...
void coincidence_creation(std::vector<TString> coincidences_filename, TString this_dir, TString write_dir, Int_t nr_datapoints, ULong64_t window, Int_t cutted_events, Int_t window_events, TString data_type_chain, TString data_type_tree, Int_t hist_bins, Double_t ns_window) {
	// Write a root file containing info on the coincidences 

	// Chain the files
	TChain* mychain = new TChain(data_type_chain);
	...
	std::cout << entry << " Entries in chain" << std::endl;
	...
	for (Double_t k = 0; k < nr_datapoints; k++) {
		...
		coinc_tree->Branch("coinc_e0", &coinc_e0, "coinc_e0/s");						// Make a trigger branch
		coinc_tree->Branch("coinc_e1", &coinc_e1, "coinc_e1/s");						// Make a coincidence branch
		...
		TH1F* htdiff = new TH1F("htdiff", "Time difference", 40, -ns_window * 1000, ns_window * 1000);
				...
				for (Int_t j = -window_events; j < window_events; j++) {				// Check window_events before and after
					...
					if (channel == 1) {													// Only check second detector
						...
						if (dt <= window) {												// If in coincidence, add event to newly created tree
							...
							coinc_time = t0;											// Fill detector 0 in coincidence histogram
							t_diff = dt;
							coinc_detector = 0;
							coinc_e0 = e0;
							coinc_e1 = e1;
							...
							coinc_time = t1;											// Fill detector 1 in coincidence histogram
							t_diff = -dt;
							coinc_detector = 1;
							coinc_e0 = e1;
							coinc_e1 = e0;
...
std::vector<Int_t> get_coincrates(TString directory, TString filename, std::vector<Double_t> channels0, std::vector<Double_t> channels1, std::vector<Int_t> hwidth) {
	TFile* f = new TFile(directory + filename);
	...
	//std::cout << nr_entries << std::endl;
	
	for (int i = 0; i < nr_entries; i++) {
		mytree->GetEntry(i);

		if (detector == 0) {
			if (abs(energy - channels0[0]) <= hwidth[0]) {
				if (abs(energy_c - channels1[1]) <= hwidth[1]) {
					N01 += 1;
				}
			}
			else if (abs(energy - channels0[1]) <= hwidth[1]) {
