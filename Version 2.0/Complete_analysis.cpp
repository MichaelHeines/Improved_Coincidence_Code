...
int main(int argc, char** argv) {
	...
	// Calculate the mean times and determine the number of datapoints per batch (can be set to 1)
	...
	for (Int_t i = 0; i < nr_dirs; i++) {
		...
		// If no secondary data has been written before
	...
	for (Int_t i = 0; i < nr_dirs; i++) {
		...
		// Loop over datapoints within the batch
		for (Int_t j = 0; j < per_batch[i]; j++) {
			second_index = std::to_string(j);
			index = indices[i] + second_index;
			coincidences_file[j] = index + coincidences_filename;
